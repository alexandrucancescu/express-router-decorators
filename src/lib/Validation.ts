...
function validatePair(propertyValidatorPair:PropertyValidatorPair,req:Request):{validated?:any,valid:boolean,error?:string}{
    let source=getSource(propertyValidatorPair.source,req);
    let data;
    let value;
    if(!source){
        throw "Should define a source for validation of "+propertyValidatorPair.property;
    }
    if(source===BodySource.HEADERS){
        value=req.get(propertyValidatorPair.property);
    }else{
        data=req[source];
        if(!data) return {valid:false,error:`Request ${data} missing!`};
        value=getValue(data,propertyValidatorPair.property);
    }
    if(value==undefined || value==null){
        return {valid:false,error:`Property ${propertyValidatorPair.property} is missing from request ${source}`}
    }

    let conditions=propertyValidatorPair.conditions;

    if(conditions.type){
        const convertFromString=source===BodySource.QUERY;
...
function getSource(source:BodySource,req:Request):BodySource|undefined{
    if(source!==null && source!==undefined){
        return source;
    }else{
        switch (req.method){
            case "POST": case "PUT": case "DELETE" : case "PATCH": return BodySource.BODY;
            case "GET": return BodySource.QUERY;
        }
    }
}
