...
function validatePair(propertyValidatorPair, req) {
    ...
    if (source === Types_1.BodySource.HEADERS) {
        value = req.get(propertyValidatorPair.property);
    }
    else {
        data = req[source];
        if (!data)
            return { valid: false, error: `Request ${data} missing!` };
    ...
    if (conditions.type) {
        const convertFromString = source === Types_1.BodySource.QUERY;
...
function getSource(source, req) {
    if (source !== null && source !== undefined) {
        ...
        switch (req.method) {
            ...
            case "PUT":
            case "DELETE":
            case "PATCH": return Types_1.BodySource.BODY;
            case "GET": return Types_1.BodySource.QUERY;
        }
    }
}
